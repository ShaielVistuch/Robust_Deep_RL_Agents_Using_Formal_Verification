...
from check_verification_group_2_in_model import marabou_loop_finder, marabou_optimal_path_finder
...
class verify_deep_q_learning_with_replay_with_reward_shaping():
    ...
    def verify_and_fix(self):
        ...
        for co in range(self.size):
            ...
            states_to_fix = marabou_optimal_path_finder(self, 4)
            try:
                for state in states_to_fix:
                    print(state)
                    self.multi_verification_fix(state, -1)
                # states_to_fix = states_to_fix[0]
            except:
                print("single")
            if states_to_fix != None:
